...
def read_line(layout_name):
	...
	for line in raw_layout:
       ...
       #new_line is a list=[ id, res_value, drop_value(mV), current_value(mA), em_value (current_density)(A/cm), layer, width, x, y, x1, y1, ] length unit is nm
			new_line=[int(line_list[1]), float(line_list[2])/10,float(line_list[5]),float(line_list[6])/1000,float(line_list[7]),int(line_list[8]),float(line_list[9]), int(line_list[10]),int(line_list[11]),int(line_list[12]),int(line_list[13])]
...
def tree_position(current_bl, direc):
	...
	for branch in sorted_branch:
		if len(tree[iterator]) == 0:
			tree[iterator].append(branch)
		else:
			last_branch = tree[iterator][-1]
...
def get_currenttree(current_bl):
	...
	for current_branch in layer_b:
		trees_samedim = tree_position(current_branch, direc)
		for one_tree in trees_samedim:
	...
	for branch in branch_list:
		if branch.wiretype == 'm':	
			...
			if branch.start not in node_position:
				...
				node_current[node_index_start]= node_current[node_index_start] + start_current
			if branch.end not in node_position:
				...
				node_current[node_index_end] = node_current[node_index_end] + end_current
...
def write_voltagesource(voltage_tree): #layer 41, connect with pad
	...
	for i in range (len(pad_node)):
		...
		nodename = 'n1_' + str(pad_node[i][0]) + '_' + str(pad_node[i][1])
		vss = 0
		vdd = 1.8
...
def write_node(nodes_position):
	
	nodes_output_str = '.print'
	nodes_output_str = nodes_output_str + ' tran'
	for node in nodes_position:
		node_str = ' v(n1_' + str(node[0]) + '_' + str(node[1]) + ')'
		nodes_output_str = nodes_output_str + node_str
	#print(nodes_output_str)
	nodes_output = [[nodes_output_str]]
	return nodes_output			 
	...
	for i in range(len(tree_layer)):
		...
		for j in range(len(tree_layer[i])): #one tree 
			for k in range (len(tree_layer[i][j])):
				...
				branch_name = 'R' + str(layer_id) + '-'+ str(100+j) + '-' + str(100 + k) 
				...
				node_name_start = 'n1' + '_' +str(branch.start[0]) + '_' +str(branch.start[1]) 	
				node_name_end = 'n1' + '_' + str(branch.end[0])+ '_' + str(branch.end[1])
...
def write_spice(layer_name, branch_layer, tree_layer, layer_type, nodes_position, node_current,filename ): 
	# layer 73 is MRDL, connect with voltage source
	voltage_layerid = layer_name.index(73)
	...
	node_output = write_node(nodes_position) 
	output_list=voltage_source_output +  tree_output + current_output + [['.op']] + node_output+ [['.end']]
	#output_list=voltage_source_output +  tree_output + [['.op']] + node_output+ [['.end']]
...
if __name__ == '__main__':
	filename = "ChipTop2.pna"
	outputfile = "chiptop.sp"
	#filename = "ChipTop_test.pna"
	#outputfile = "chiptop_test.sp"
	#filename = "test.pna"
	#outputfile = "armtest.sp"
	...
	for i in range(len(node_current)):
		node_current[i] = node_current[i] * 500
