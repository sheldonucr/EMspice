#!/usr/bin/env python3
# -*- coding: utf-8 -*-
file1=open('data/u_stress_20.txt',encoding='utf-8')
file2=open('temp2/stress.txt','w',encoding='utf-8')
...
for line in file1:
    ...
    if(line[0]=='R'):# 'line' here is a string
        ...
        for i in range(3):
           ...
           file2.write(' ')
        file2.write('\n')
        s=('temp2/stress'+str(branch[0])+'_'+str(branch[1]+'_'+str(branch[2])+'.txt'))
        file=open(s,'w',encoding='utf-8')
        file.write(str(int(float(line_list[1])/1000000)))
        file.write('\n')        
    else:
        line_list=line.split( )
        file.write(str(int(float(line_list[0])/1000000)))
        file.write('\n') 
